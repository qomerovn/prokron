...
class ProkronCLITest(unittest.TestCase):
    ...
    def test_realistic_fixture_end_to_end_and_checkpoint(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("T-003\tWrite reviewer guidance", self.run_cli(project, "next").stdout)
            ...
            rejected = self.run_cli(
                project, "checkpoint", "--task", "T-002", "--did", "", "--validation", "Not run.",
                "--left-mid-air", "Still working.", "--next", "Continue.",
            )
            self.assertEqual(rejected.returncode, 2)
            self.assertEqual(intent_path.read_text(encoding="utf-8"), intent_before)
            comma_path = self.run_cli(
                project, "checkpoint", "--task", "T-002", "--did", "Checked input.", "--validation", "Not run.",
                "--left-mid-air", "Still working.", "--next", "Continue.", "--changed-file", "src/a.py,src/b.py",
            )
            ...
            checkpoint = self.run_cli(
                project, "checkpoint", "--task", "T-002", "--did", "Added invalid-transition checks.",
                "--validation", "Workflow tests pass.", "--left-mid-air", "Ready for review; not human-verified.",
                "--next", "Ask a reviewer to verify the transition contract.", "--changed-file", "tests/test_workflow.py",
            )
    ...
    def test_empty_markers_without_final_newlines_are_removed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for did in ("No journal entries.", "Added another entry."):
                checkpoint = self.run_cli(
                    project, "checkpoint", "--did", did, "--validation", "Regression test.",
                    "--left-mid-air", "Done.", "--next", "Review.",
                )
                self.assertEqual(checkpoint.returncode, 0, checkpoint.stderr)
    ...
    def test_checkpoint_rejects_missing_intent_and_multiline_input(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            missing = self.run_cli(
                project, "checkpoint", "--did", "Nothing.", "--validation", "Not run.",
                "--left-mid-air", "Idle.", "--next", "Create a task.",
            )
            self.assertEqual(missing.returncode, 2)
            fixture = Path(temporary) / "greenfield"
            shutil.copytree(PROJECT_ROOT / "examples" / "greenfield", fixture)
            multiline = self.run_cli(
                fixture, "checkpoint", "--did", "first line\nsecond line", "--validation", "Not run.",
                "--left-mid-air", "Still working.", "--next", "Continue.",
            )
    ...
    def test_adapters_delegate_to_the_cli(self) -> None:
        for relative_path in ("adapters/codex/AGENTS.md", "adapters/claude/CLAUDE.md", "adapters/generic/README.md"):
            self.assertIn("prokron", (PROJECT_ROOT / relative_path).read_text(encoding="utf-8"))
